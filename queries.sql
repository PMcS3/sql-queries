...
SELECT * FROM invoices WHERE billing_city = 'Mountain View' AND billing_state = 'CA' ORDER BY total DESC LIMIT 5;
...
SELECT billing_state, COUNT(*), SUM(total), AVG(total) FROM invoices WHERE billing_country = 'USA' GROUP BY billing_state ORDER BY AVG(TOTAL);
...
SELECT billing_country, SUM(total) FROM invoices GROUP BY billing_country ORDER BY SUM(total) LIMIT 5;
-- A list of the top 5 countries by average invoice size
SELECT billing_country, AVG(total) FROM invoices GROUP BY billing_country ORDER BY AVG(total) LIMIT 5;
